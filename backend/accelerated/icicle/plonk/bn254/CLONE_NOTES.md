...
| 4 | `@@ -75,44 +107,103 @@` | (b)(c)(d) | `type Proof` DELETED (the clone constructs the native `plonk_bn254.Proof` so serialization/Verify work unchanged); seam declarations added: `var randFr = defaultRandFr` + `defaultRandFr` (crypto/rand default, tag contract documented), `var onStageCheckpoint` + nil-guarded `checkpoint` helper; `Prove` signature → `Prove(spr, pk *ProvingKey, fullWitness, opts *icicle.Config) (*plonk_bn254.Proof, error)`; log gains `Str("acceleration", "icicle")`; `backend.NewProverConfig(opts.ProverOpts...)`; device-setup block added (gpuinit.WarmUpOnce → CreateDevice → `pk.acquireDeviceSRS(&device, opts.PinToGPU)` BEFORE the solver per DESIGN §5 → paired `defer pk.releaseDeviceSRS()`). The acquire/release pair is the refcounted device-SRS lifetime model (provingkey.go): each Prove holds one reference, the LAST release frees unless the key is pinned (pin = set-once promotion from pk.PinToGPU or opts.PinToGPU, read under setupMu — Prove never writes pk.PinToGPU). The release-at-exit is sound only under the doc.go invariant "no GPU work outside the errgroup tasks". `newInstance` call gains `&device` |
| 5 | `@@ -157,10 +248,14 @@` | (b) | instance fields: `proof *plonk_bn254.Proof`; new `device icicle_runtime.Device` (consumed by the class-(f) MSM call sites) |
| 6 | `@@ -199,17 +294,18 @@` | (b) | `trace *plonk_bn254.Trace`; `newInstance` gains the `device *icicle_runtime.Device` param; struct literal sets `device: *device`, `proof: &plonk_bn254.Proof{}` |
| 7 | `@@ -236,9 +332,15 @@` | (c) | seam site 2+3: `quotientShardsRandomizers[0/1]` ← `randFr("qsr0"/"qsr1", 0)` with error propagation (upstream ignored the SetRandom error) |
| 8 | `@@ -251,16 +353,31 @@` | (b)(c)(d) | `plonk_bn254.NewTrace`; `initBlindingPolynomials` rewritten: `getRandomPolynomial(order, tag)` with tags `"bl"/"br"/"bo"/"bz"` + error propagation (seam site 1, four polynomials); checkpoints `bp-bl/br/bo/bz` |
| 9 | `@@ -290,14 +407,20 @@` | (c)(f) | seam sites 4+5: the two bsb22Hint blinding draws ← `randFr(fmt.Sprintf("bsb22-%d", commDepth), 0/1)` (same target slots, same comments); **row 4**: `kzg.Commit(..., s.pk.KzgLagrange)` → `gpuCommit(&s.device, ..., s.pk.KzgLagrange.G1, s.pk.G1Device.KzgLagrange)` — fires inside `spr.Solve` (§5 ordering invariant) |
| 10 | `@@ -340,6 +463,10 @@` | (d) | checkpoints `solution-l/r/o` after `wg.Wait()`, before commitToLRO |
| 11 | `@@ -435,7 +562,9 @@` | (f) | **row 1** (L wire): `commit.MultiExp(s.pk.KzgLagrange.G1[:offset], coeffs[:offset], ...)` → `commit, err = gpuMsm(&s.device, coeffs[:offset], s.pk.KzgLagrange.G1, s.pk.G1Device.KzgLagrange, 0)`; the in-place −s0/+s0 bracket, correctionPoint and commitBlindingFactor around it are untouched |
| 12 | `@@ -454,7 +583,9 @@` | (f) | **row 2** (R wire): interior window → `gpuMsm(&s.device, coeffs[nbPublic:offset], s.pk.KzgLagrange.G1, s.pk.G1Device.KzgLagrange, nbPublic)` |
| 13 | `@@ -473,7 +604,9 @@` | (f) | **row 3** (O wire): identical to row 2 |
| 14 | `@@ -518,6 +651,9 @@` | (d) | checkpoints `gamma`, `beta` before `close(s.chGammaBeta)` |
| 15 | `@@ -529,7 +665,9 @@` | (f) | **row 5** (grand-product Z via commitToPolyAndBlinding): `kzg.Commit(p.Coefficients(), s.pk.KzgLagrange)` → `gpuCommit(&s.device, p.Coefficients(), s.pk.KzgLagrange.G1, s.pk.G1Device.KzgLagrange)`; blinding commit stays CPU |
| 16 | `@@ -594,6 +732,7 @@` | (d) | checkpoint `alpha` after deriveAlpha |
| 17 | `@@ -605,20 +744,25 @@` | (d)(f) | checkpoints `numerator`, `h`, `zeta` (former P3.5 hunks 13+14, fused by the interleaved row 6–8 call site); **rows 6–8 call site**: `commitToQuotient(s.h1(), s.h2(), s.h3(), s.proof, s.pk.Kzg)` → `commitToQuotient(&s.device, ..., s.pk.Kzg, s.pk.G1Device.Kzg)` |
| 18 | `@@ -657,6 +801,7 @@` | (d) | checkpoint `z-lagrange` after BuildRatioCopyConstraint |
| 19 | `@@ -678,10 +823,14 @@` | (d)(f) | **row 9** (Z-shift opening): `kzg.Open(s.blindedZ, zetaShifted, s.pk.Kzg)` → `gpuOpen(&s.device, s.blindedZ, zetaShifted, s.pk.Kzg.G1, s.pk.G1Device.Kzg)`; checkpoint `blinded-z` before `close(s.chZOpening)` |
| 20 | `@@ -783,9 +932,13 @@` | (d)(f) | checkpoint `linearized-poly`; **row 10**: `kzg.Commit(s.linearizedPolynomial, s.pk.Kzg, runtime.NumCPU()*2)` → `gpuCommit(&s.device, s.linearizedPolynomial, s.pk.Kzg.G1, s.pk.G1Device.Kzg, runtime.NumCPU()*2)` (nbTasks accepted and ignored) |
| 21 | `@@ -824,16 +977,25 @@` | (d)(f) | **row 11** (batch opening): `kzg.BatchOpenSinglePoint(polysToOpen, digestsToOpen, s.zeta, s.kzgFoldingHash, s.pk.Kzg, s.proof.ZShiftedOpening.ClaimedValue.Marshal())` → `gpuBatchOpenSinglePoint(&s.device, polysToOpen, digestsToOpen, s.zeta, s.kzgFoldingHash, s.pk.Kzg.G1, s.pk.G1Device.Kzg, s.proof.ZShiftedOpening.ClaimedValue.Marshal())` — same dataTranscript bytes; tail split into err-check + checkpoint `batch-claimed-values` + `return nil` |
| 22 | `@@ -1236,7 +1398,9 @@` | (c) | `getRandomPolynomial(n int, tag string) (*iop.Polynomial, error)` — signature gains the tag and an error |
| 23 | `@@ -1244,12 +1408,15 @@` | (c) | seam site 1 body: `a[i], err = randFr(tag, i)` (one draw per coefficient, i = coefficient index); `return res, nil` |
| 24 | `@@ -1260,21 +1427,26 @@` | (b)(f) | **rows 6–8 body**: `commitToQuotient` signature gains `device *icicle_runtime.Device` (front) and `devKzg icicle_core.DeviceSlice` (back), `proof *plonk_bn254.Proof`; the three `kzg.Commit(h_i, kzgPk)` → `gpuCommit(device, h_i, kzgPk.G1, devKzg)` inside the same errgroup |
| 25 | `@@ -1487,3 +1659,85 @@` | (e) | `bindPublicData` (verify.go:315-359) + `deriveRandomness` (verify.go:361-379) appended with provenance comments. Byte-identical to upstream except the `vk *plonk_bn254.VerifyingKey` parameter qualification (mechanically verified, see below) |
...
|---|---|---|---|
| 1 | `commitToLRO`, line 567 | `gpuMsm(&s.device, coeffs[:offset], ..., 0)` | Lagrange |
| 2 | `commitToLRO`, line 588 | `gpuMsm(&s.device, coeffs[nbPublic:offset], ..., nbPublic)` | Lagrange |
| 3 | `commitToLRO`, line 609 | same as row 2 | Lagrange |
| 4 (×c) | `bsb22Hint`, line 423 | `gpuCommit(&s.device, s.cCommitments[commDepth].Coefficients(), ...)` | Lagrange |
| 5 | `commitToPolyAndBlinding`, line 670 | `gpuCommit(&s.device, p.Coefficients(), ...)` | Lagrange |
| 6–8 | `commitToQuotient`, lines 1439/1444/1449 | three `gpuCommit(device, h_i, kzgPk.G1, devKzg)` | canonical |
| 9 | `openZ`, line 829 | `gpuOpen(&s.device, s.blindedZ, zetaShifted, ...)` | canonical |
| 10 | `computeLinearizedPolynomial`, line 941 | `gpuCommit(&s.device, s.linearizedPolynomial, ..., runtime.NumCPU()*2)` | canonical |
| 11 | `batchOpening`, line 983 | `gpuBatchOpenSinglePoint(&s.device, ...)` | canonical |

Stays CPU forever (DESIGN.md §3): `commitBlindingFactor` (line 1385, two
size-≤3 MultiExps at lines 1391/1395), `correctionPoint.ScalarMultiplication`
(commitToLRO, line 554), all FFT/closure code.
...
The per-curve `provingkey.go` must replicate the bn254 device-SRS lifetime
model verbatim: refcounted acquire/release under `setupMu` (one reference per
in-flight Prove; the last release frees unless pinned), device-identity check
