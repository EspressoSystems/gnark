...
func TestProveDeterministicSeededRandomness(t *testing.T) {
	ccs, err := frontend.Compile(ecc.BLS12_381.ScalarField(), scs.NewBuilder, &proveTestCircuit{})
	if err != nil {
	...
	pk, vk, err := Setup(spr, *srs.(*kzg_bls12381.SRS), *srsLagrange.(*kzg_bls12381.SRS))
	if err != nil {
	...
	run := func() ([]byte, map[string]string) {
		...
		if err := plonk_bls12381.Verify(proof, vk, pw.Vector().(fr.Vector)); err != nil {
...
// unsafe KZG SRS.
func compileSetup(t *testing.T, circuit frontend.Circuit) (*cs.SparseR1CS, *ProvingKey, *plonk_bls12381.VerifyingKey) {
	t.Helper()
...
func proveVerifySerialize(t *testing.T, spr *cs.SparseR1CS, pk *ProvingKey, vk *plonk_bls12381.VerifyingKey, w witness.Witness, pubW fr.Vector, cfg *icicle.Config) []byte {
	t.Helper()
	proof, err := Prove(spr, pk, w, cfg)
	if err != nil {
		t.Fatalf("prove: %v", err)
	}
	if err := plonk_bls12381.Verify(proof, vk, pubW); err != nil {
		t.Fatalf("native verify rejected the icicle proof: %v", err)
	}
	var buf bytes.Buffer
	if _, err := proof.WriteTo(&buf); err != nil {
...
func concurrentProvePair(t *testing.T, spr *cs.SparseR1CS, pk *ProvingKey, vk *plonk_bls12381.VerifyingKey, w witness.Witness, pubW fr.Vector, pinFlags [2]bool) int {
	t.Helper()
	...
	var proofs [2]*plonk_bls12381.Proof
	var errs [2]error
	for i := 0; i < 2; i++ {
	...
	for i := 0; i < 2; i++ {
		...
		if err := plonk_bls12381.Verify(proofs[i], vk, pubW); err != nil {
