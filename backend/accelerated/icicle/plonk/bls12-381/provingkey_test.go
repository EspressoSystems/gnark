...
func setupTestPk(t *testing.T) *ProvingKey {
	t.Helper()
	ccs, err := frontend.Compile(ecc.BLS12_381.ScalarField(), scs.NewBuilder, &pkTestCircuit{})
	if err != nil {
	...
	pk, vk, err := Setup(ccs.(*cs.SparseR1CS), *srs.(*kzg_bls12381.SRS), *srsLagrange.(*kzg_bls12381.SRS))
	if err != nil {
