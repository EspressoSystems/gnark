...
// It wraps the native [plonk_bls12377.Setup]; the returned proving key shares the
...
func Setup(spr *cs.SparseR1CS, srs, srsLagrange kzg.SRS) (*ProvingKey, *plonk_bls12377.VerifyingKey, error) {
	nativePk, vk, err := plonk_bls12377.Setup(spr, srs, srsLagrange)
	if err != nil {
		return nil, nil, err
	}
	pk := &ProvingKey{}
