...
type kzgTestEnv struct {
	...
	canonical *kzg_bw6761.SRS
	lagrange  *kzg_bw6761.SRS
...
func setupKzgTest(t *testing.T) *kzgTestEnv {
	...
	ccs, err := frontend.Compile(ecc.BW6_761.ScalarField(), scs.NewBuilder, &kzgTestCircuit{})
	if err != nil {
	...
	canonical := srs.(*kzg_bw6761.SRS)
	lagrange := srsLagrange.(*kzg_bw6761.SRS)
...
func assertDigestEqual(t *testing.T, what string, got, want kzg_bw6761.Digest) {
	t.Helper()
...
func assertCommitParity(t *testing.T, env *kzgTestEnv) {
	...
	for _, b := range []struct {
		...
		for _, size := range b.sizes {
			t.Run(fmt.Sprintf("%s/size=%d", b.name, size), func(t *testing.T) {
				...
				want, err := kzg_bw6761.Commit(p, b.pk)
				if err != nil {
	...
	t.Run("canonical/nbTasks", func(t *testing.T) {
		...
		want, err := kzg_bw6761.Commit(p, env.canonical.Pk, 16)
		if err != nil {
			t.Fatalf("kzg.Commit: %v", err)
		}
		assertDigestEqual(t, "commitment", got, want)
	...
	t.Run("canonical/oversized", func(t *testing.T) {
		p := randomFr(t, n+4)
		if _, err := gpuCommit(env.device, p, env.canonical.Pk.G1, env.devCan); !errors.Is(err, kzg_bw6761.ErrInvalidPolynomialSize) {
...
func assertOpenParity(t *testing.T, env *kzgTestEnv) {
	...
	for _, size := range []int{1, 17, 65, n, n + 2, n + 3} {
		t.Run(fmt.Sprintf("size=%d", size), func(t *testing.T) {
			...
			want, err := kzg_bw6761.Open(p, point, env.canonical.Pk)
			if err != nil {
			...
			digest, err := kzg_bw6761.Commit(p, env.canonical.Pk)
			if err != nil {
				t.Fatalf("kzg.Commit: %v", err)
			}
			if err := kzg_bw6761.Verify(&digest, &got, point, env.canonical.Vk); err != nil {
	...
	t.Run("oversized", func(t *testing.T) {
		p := randomFr(t, n+4)
		if _, err := gpuOpen(env.device, p, randomFr(t, 1)[0], env.canonical.Pk.G1, env.devCan); !errors.Is(err, kzg_bw6761.ErrInvalidPolynomialSize) {
			t.Fatalf("want ErrInvalidPolynomialSize, got %v", err)
		}
	})
}
...
func assertBatchOpenParity(t *testing.T, env *kzgTestEnv) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			digests := make([]kzg_bw6761.Digest, len(tc.sizes))
			for i, size := range tc.sizes {
				polys[i] = randomFr(t, size)
				var err error
				digests[i], err = kzg_bw6761.Commit(polys[i], env.canonical.Pk)
				if err != nil {
			...
			want, err := kzg_bw6761.BatchOpenSinglePoint(polys, digests, point, sha256.New(), env.canonical.Pk, dataTranscript...)
			if err != nil {
			...
			if err := kzg_bw6761.BatchVerifySinglePoint(digests, &got, point, sha256.New(), env.canonical.Vk, dataTranscript...); err != nil {
	...
	t.Run("digestCountMismatch", func(t *testing.T) {
		...
		digests := make([]kzg_bw6761.Digest, 1)
		if _, err := gpuBatchOpenSinglePoint(env.device, polys, digests, randomFr(t, 1)[0], sha256.New(), env.canonical.Pk.G1, env.devCan); !errors.Is(err, kzg_bw6761.ErrInvalidNbDigests) {
	...
	t.Run("oversized", func(t *testing.T) {
		...
		digests := make([]kzg_bw6761.Digest, 1)
		if _, err := gpuBatchOpenSinglePoint(env.device, polys, digests, randomFr(t, 1)[0], sha256.New(), env.canonical.Pk.G1, env.devCan); !errors.Is(err, kzg_bw6761.ErrInvalidPolynomialSize) {
			t.Fatalf("want ErrInvalidPolynomialSize, got %v", err)
		}
	})
}
