...
func setupTestPk(t *testing.T) *ProvingKey {
	t.Helper()
	ccs, err := frontend.Compile(ecc.BW6_761.ScalarField(), scs.NewBuilder, &pkTestCircuit{})
	if err != nil {
	...
	pk, vk, err := Setup(ccs.(*cs.SparseR1CS), *srs.(*kzg_bw6761.SRS), *srsLagrange.(*kzg_bw6761.SRS))
	if err != nil {
